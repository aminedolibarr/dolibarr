...
class Mo extends CommonObject
{
	...
	public $fields = array(
		...
		'mrptype' => array('type'=>'integer', 'label'=>'Type', 'enabled'=>1, 'visible'=>1, 'position'=>34, 'notnull'=>1, 'default'=>'0', 'arrayofkeyval'=>array(0=>'Manufacturing', 1=>'Disassemble',2=>"Traitement"), 'css'=>'minwidth150', 'csslist'=>'minwidth150 center'),
...
//
//
//        $s =" SELECT * FROM `llx_mrp_production` WHERE 1";
//        $resql = $this->db->query($s);
//
//        var_dump($resql);
		...
		// Product or sub-bom
		// if the label  AS pr DE show the selection column


        if(isset($_SESSION['bomType'])){


        if ($_SESSION['bomType'] != 2) {
            if($_SESSION['bomType'] == 1){
                //show only on the disasseblage
                echo '<td class="center">' . $langs->trans('Manquante'). '</td>';
            }
            echo '<td class="center">' . $langs->trans('Selection'). '</td>';
...
//				 print_r($this->lines);
    ...
    private function accessoirPrintLine(){
    ...
    /**
     * Checks if a product is considered finished based on its row ID.
     *
     * @param int $fk_product The row ID of the product to check.
     * @return bool Returns true if the product is finished (finished equals 2), otherwise false.
     * @author bilal
     */
    public function testProduct($fk_product) {
        $sql = "SELECT finished FROM llx_product WHERE rowid = " . (int)$fk_product;
        $resql = $this->db->query($sql);

        if ($resql && $this->db->num_rows($resql) > 0) {
            $row = $this->db->fetch_array($resql);
            if ($row && isset($row['finished'])) {
                return ($row['finished'] == 2); // Returns true if finished is 2, else false
            }
        }

        return false; // Return false if the query fails or if finished is not 2
    }

    /**
     * Determines if a given label belongs to a specific type.
     *
     * @param string $label The label to analyze.
     * @return bool Returns true if the first two letters of the label are "AS", otherwise false.
     *@author bilal
     */
	...
	public function printOriginLine($line, $var, $restrictlist = '', $defaulttpldir = '/core/tpl', $selectedLines = array())
    {
        ...
        // we test if the label is AS to print only accessoir

        if($this->bom->bomtype==0){
            // we check if the product has the accessoir type
        if ($this->testProduct($line->fk_product)) {

            $this->tpl['id'] = $line->id;

            $this->tpl['label'] = '';
            $this->tpl['selection'] = $line->fk_product;
