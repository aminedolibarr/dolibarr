...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if (!empty($object->table_element_line)) {
        ...
        if(isset($_SESSION['unchekedProduct']) && isset($_SESSION['bomType'])){
            $d =   $_SESSION['unchekedProduct'] ;
            $uncheckedProduct = explode(',', $d);
            $uncheckedProduct = array_unique($uncheckedProduct);

            $bomType = $_SESSION['bomType'];

        if ($_SESSION['bomType']==0) {
            ...
            if (!empty($object->lines) ) {
                ...
                foreach ($object->lines as $line) {
                    if ($line->role == 'toconsume') {
                        ...
                        if (!in_array($line->fk_product, $uncheckedProduct)) {
        ...
        if (isset($_COOKIE['DELSESSIDS_6489c7a8a26573c0']) && $_SESSION['bomType']==1) {
                $d = $_COOKIE['DELSESSIDS_6489c7a8a26573c0'];
                $idsProdDes = explode(',', $d);
            $idsProdDes = array_unique($idsProdDes);
		...
		if (!empty($object->lines)) {
			...
			foreach ($object->lines as $line) {
				if ($line->role == 'toproduce') {
                    if (!empty($idsProdDes)) {
                    if ( count($idsProdDes) >= $nblinetoproducecursor++) {
                        $i = 1;
